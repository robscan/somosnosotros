...
- **Correos y cron activos (2026-09-14)**: `RESEND_API_KEY`, `CORREO_REMITENTE`, `CRON_SECRET` en Vercel (Production); dominio verificado en Resend; correo de prueba entregado; `/api/recordatorios` con el secreto → `{"ok":true,"eventos":1,"enviados":1}`. Si se quiere mapa/avisos en los previews de PR, replicar las variables en Preview.
