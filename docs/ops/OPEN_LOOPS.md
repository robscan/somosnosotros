...
- **Propuestas abiertas** (el founder decide): (a) llenar un evento desde la foto del cartel con un modelo de visión; necesita una llave de API de Anthropic en Vercel como variable secreta. (b) Eventos en "otro sitio" sin dar de alta un lugar (lugar opcional + texto + pin opcional); cambio pequeño en la base y el formulario; esos eventos no ponen pin en el mapa de lugares.
