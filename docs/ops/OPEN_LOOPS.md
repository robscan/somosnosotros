...
- Fase 1: ya hay una segunda persona registrada (2026-09-14, entró con su correo). **Correos de entrar por Resend**: SMTP propio en Supabase (smtp.resend.com:465, usuario `resend`, remitente avisos@somosnosotros.org) configurado por el founder; primer intento falló por la contraseña, corregido y verificado (enlace enviado sin error). Plantillas en español con `token_hash` (el enlace funciona aunque se abra en otro navegador). Entregabilidad: SPF y DKIM de Resend en el DNS de Vercel; DMARC `p=none` agregado por el founder (2026-09-14) y resolviendo; el primer correo a Gmail cayó en no deseado (dominio nuevo): marcar "No es spam" y esperar historial. El dominio no recibe correo (sin MX en la raíz): los reportes DMARC a avisos@ no llegan a ningún buzón; si se quieren, apuntar `rua` a un buzón real o activar Inbound en Resend. **Google** opcional: credenciales OAuth en Google Cloud con redirect `https://<ref>.supabase.co/auth/v1/callback`; Client ID y Secret en Supabase → Authentication → Providers → Google.
