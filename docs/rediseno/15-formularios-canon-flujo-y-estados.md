...
11. **Elegir en dos niveles, cuando una opción abre sus propias opciones** (founder, 2026-09-25, OL-206: "Qué hace" de Registrar artista — elegir Artes visuales entre las disciplinas debía marcar una división y preguntar la subcategoría aparte, en vez de sumar chips sueltos sin explicar por qué). Paso 1: todas las opciones, como cualquier chip del canon. Al elegir una, paso 2: se ocultan las demás y la elegida queda sola, con su ✕ (mismo dibujo que el chip elegido de `ui/ChipFecha`); debajo, una línea divide y aparece la pregunta del segundo nivel, con el nombre propio de lo elegido ("¿Qué tipo de artes visuales?", no una genérica) y sus propios chips. Elegir ahí cierra el renglón con el resumen ("Artes visuales · Grabado"); la ✕ deshace los dos pasos a la vez, no uno por uno. Sin opciones conocidas para el segundo nivel, se respeta lo que ya hiciera ese renglón (aquí, ir directo al texto libre) en vez de forzar una pregunta vacía. Construido en `FormularioArtista.tsx` (`docs/rediseno/27`, subcategorías de disciplina). *Progressive disclosure, Similitud (mismo dibujo que un patrón ya conocido), Evidencia (la ✕ dice qué deshace).*
