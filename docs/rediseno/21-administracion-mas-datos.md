...
**`indicadores_diarios`:** la foto del día la guarda el cron de cada mañana (`/api/recordatorios` → `guardar_indicadores()`, decisión 4 de [19](19-administracion-flujo-y-estados.md)) y, si ese día falla, el primer vistazo a `/admin` la deja como respaldo (`panel_resumen()`, `on conflict (dia) do nothing`); solo queda un hueco si fallan las dos cosas el mismo día.
