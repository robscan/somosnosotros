...
## Carga progresiva y esqueletos (canon, OL-158, bitácora 193)

El founder, 2026-09-23: «¿ninguna sección carga con skeletons? pensé que lo teníamos como estándar»; precisó que quiere carga progresiva, que la persona vea la primera línea de contenido ya cargada y el resto llegue después (perezoso). Regla, para toda pantalla con una lista o una ficha:

- **Qué pinta primero.** La barra superior y la navegación inferior nunca esperan una consulta: van fuera de cualquier `<Suspense>`. Lo que sí depende de una consulta (la cabecera con datos reales, la lista, el cuerpo de una ficha) vive dentro de su propio `<Suspense>`, con un esqueleto de `fallback` del mismo tamaño exacto de lo real — nunca texto real, nunca un salto al llegar la respuesta.
- **Un solo origen de esqueleto.** `src/components/ui/Esqueleto.tsx`: `EsqueletoRenglon`/`EsqueletoRenglones` (una fila de listado), `EsqueletoTarjeta` (tarjeta de carril, grande/mediana/chica — la usa `CarrilEsqueleto` de Inicio), `EsqueletoCabeceraFicha` (foto, nombre, dos líneas de meta) y `EsqueletoBloqueTexto`. Todos con el mismo pulso (`animation: respirar`), apagado con `prefers-reduced-motion`. `src/components/ListaEsqueleto.tsx` compone una cabecera genérica (chips, pestañas) más `EsqueletoRenglones`: el `fallback` de Agenda y Artistas.
- **Listas largas, en tandas.** La primera tanda (`TANDA_INICIAL`, 20) se pinta de inmediato; las siguientes se revelan solas al acercarse al final del scroll (`useCentinela`, `IntersectionObserver` con 600 px de margen) o, en Artistas (que pagina en el servidor con `n`), pidiendo la página siguiente sola. `ui/CargarMas` deja "Ver más" siempre en el árbol como respaldo accesible — sin `IntersectionObserver`, o para quien no dispara el scroll al final. La lógica de cuántos mostrar es pura (`src/lib/tandas.ts`: `tandaInicial`, `siguienteTanda`, `tandaAcotada`) y se prueba sola, sin DOM. La memoria de pantalla (`useMemoriaPantalla`) repone cuántos iban mostrados, igual que la pestaña o la búsqueda: volver de una ficha no colapsa la lista a la primera tanda.
- **El cargador de página completa (el símbolo SN) queda solo para rutas sin lista**: perfil, admin, entrar, alta y edición de lugar/evento/artista. Cualquier ruta con una lista o una ficha usa el canon de arriba, no `app/loading.tsx`.
- **Estado (2026-09-23):** Agenda y Artistas ya pintan la barra al instante y su lista en `<Suspense>` con `ListaEsqueleto`; sus listas reparten en tandas (Agenda por render, sobre los eventos ya cargados; Artistas pidiendo la página siguiente sola). Lugares reparte su lista en tandas (mismo `useCentinela`/`ui/CargarMas`), pero su cabecera y su mapa siguen esperando la consulta entera (`VistaLugares` arma su propio `<main>` con la barra dentro, sin separación todavía) — pendiente. Las fichas de evento, lugar y artista siguen enteras fuera de este canon — pendiente.
