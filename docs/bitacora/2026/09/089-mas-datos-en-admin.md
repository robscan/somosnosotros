...
1. **`indicadores_diarios` no se guarda garantizado "cada mañana".** Lo intenta el cron de las 9:00 (`/api/recordatorios` → `guardar_indicadores()`) y, si ese día aún no dejó la foto, el primer vistazo de un administrador la deja como respaldo (`panel_resumen()`, `on conflict (dia) do nothing`, migración `20260917100000_zona_horaria.sql`). Un día sin ninguna de las dos cosas se queda sin foto. Corregido en el documento 21. *(Este punto lo trajo gestión de cambios; en su siguiente mensaje avisó que se había equivocado — el cron sí llama a `guardar_indicadores()` cada mañana como decía la versión original, y el hueco solo pasa si fallan el cron **y** el vistazo del día. Se deshizo la propuesta de un cron aparte que se había sumado aquí por esa corrección, y quedó una sola línea con lo de arriba.)*
