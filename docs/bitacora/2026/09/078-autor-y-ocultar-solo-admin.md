# 078 · El autor de una ficha y ocultarla, solo la administración (OL-051)
...
## Qué se hizo
- **Migración `supabase/migrations/20260917120000_autor_y_visible_solo_admin.sql`, sin aplicar.** Va después de `20260917100000_zona_horaria` y `20260917110000_lectura_al_crear`. Crea la función `proteger_autor_y_visible()` y un trigger con ese nombre antes de cada UPDATE de `creado_por` o `visible` en `lugares`, `artistas` y `eventos`, como `proteger_rol`:
...
- **Banco nuevo:**
  ...
  - 68 con la zona horaria y `lectura_al_crear` aplicadas antes, en el orden en que irán a producción. Sus migraciones se copiaron de los árboles `cualquier-pais` y `reverent-germain-8bb38f` y son iguales a sus commits;
...
- **Bancos de las otras piezas con esta migración:** el del panel da 95 comprobaciones y el de `lectura_al_crear`, 42 (con la zona horaria antes).
...
## Queda
- **Aplicar la migración** `20260917120000_autor_y_visible_solo_admin.sql` cuando lo indique gestión de cambios, después de `20260917110000_lectura_al_crear.sql`. Se puede aplicar antes de mezclar, porque el código de hoy sigue funcionando. Mientras no se aplique, el hueco sigue abierto en producción.
