...
## Corrección del founder (2026-09-23, sobre esta misma pieza)

Con la tarjeta ya de vuelta (punto 4), el founder corrigió el botón: fuera el botón de texto («Ver ficha»/«Ver mi ficha de artista»); en su lugar, a la derecha, el botón de compartir con el canon accionable circular de las fichas (círculo elevado con la sombra de siempre, icono de compartir, sin letrero). Tocar la tarjeta va a la ficha del artista; tocar compartir comparte directo (la misma hoja de compartir de la ficha), sin navegar — el botón es hermano del enlace, no su hijo, como en `Destacados`, para que el toque no se propague. Pidió retirar `etiquetaVerMiFicha` si quedaba sin uso.

- `src/app/perfil/MisArtistas.tsx`: la tarjeta pasa a ser un grid de dos columnas — el enlace (`.frente`, con la foto y el nombre) y el compartir, hermanos, nunca uno anidado en el otro — el mismo patrón de `Renglon.module.css` (`.renglon`/`.frente`) que ya usa `RenglonArtista`. El botón «Ver mi ficha de artista»/«Ver ficha» desaparece; en su lugar, `CompartirFicha` con `className={ficha.accionIcono}` (el círculo de 56 px de `ui/Ficha.module.css`, punto 1 de esta misma bitácora) — se reusa el mismo canon en vez de crear un estilo de círculo propio para la misma idea de botón, lo que el founder ya había pedido evitar en OL-159.
- `src/app/perfil/MisArtistas.module.css`: `.tarjeta` vuelve a un grid de dos columnas (`minmax(0, 1fr) auto`), como `.renglon`; `.frente` es el enlace interno (foto + nombre); se borró `.verFicha`.
- `src/app/perfil/page.tsx`: `CompartirFicha` necesita el QR ya dibujado (`svg`) y la URL completa por artista — se repuso el cálculo que había antes de OL-159 (`qrDeUrl` por artista ligado, `Promise.all`, mismo patrón que Pincel), ahora con el tipo `ArtistaLigadoConQr` que ya traía `MisArtistas.tsx`.
- `src/lib/artistas.ts`: `etiquetaVerMiFicha` se retiró (sin más uso tras quitar el botón de texto); su prueba en `src/lib/artistas.test.ts` se retiró con ella.

**Pruebas:** `npm run lint && npm run typecheck && npm test && npm run build`, verdes; **1123 pruebas** (1124 de la entrega anterior, menos la de `etiquetaVerMiFicha` que se fue con la función). **Verificado con Chrome real** (mismo arnés temporal, recreado y vuelto a borrar antes de comitear): el botón de compartir mide 56×56 px con la sombra `0 2px 12px rgba(0,0,0,.08)` (sin cambios), sin texto (`textContent` vacío); es hermano del `<a>` de la tarjeta, no su descendiente (comprobado con `boton.parentElement === link.parentElement && !link.contains(boton)`, `true` en ambas tarjetas); un clic en el botón abre la hoja de compartir (`Compartir` visible) y la URL de la página no cambia — no navega.
...
`src/components/ui/Ficha.module.css`, `src/app/artistas/[id]/page.tsx`, `src/app/lugares/[id]/page.tsx`, `src/app/eventos/[id]/page.tsx`, `src/lib/ficha.ts` (nuevo), `src/lib/ficha.test.ts` (nuevo), `src/lib/artistas.ts`, `src/lib/artistas.test.ts`, `src/app/perfil/MisArtistas.tsx`, `src/app/perfil/MisArtistas.module.css`, `src/app/perfil/page.tsx`, `docs/rediseno/capturas-198/` (4 PNG), esta bitácora y `docs/ops/OPEN_LOOPS.md`.
