...
### Corrección del gestor: el grosor no se veía (2026-09-21)

El gestor abrió los cinco estados: movimiento del botón, freno de trazos, rejilla estable y caret, bien; pero «el grosor no se ve. El punto blanco pasa de ~10 a ~12 px; en "tras soltar" es indistinguible de reposo, así que la persona no sabe qué grosor lleva». Tenía razón: escalé un glifo «●» de unos 10 px entre 0.8 y 1.25 — un cambio de matiz, no una medida. Pidió que el punto blanco **mida** el grosor a escala del mando: mínimo ~8 px, máximo ~56 px, continuo mientras se arrastra y persistente al soltar, sin tocar el botón ni la rejilla, y ajustar el rango de `grosor` para que el máximo tenga sentido en la pared.

**Cómo quedó:** el punto es un `<span>` redondo (`border-radius: 50%`, `background: currentColor`) cuyo `width`/`height` los pone `diametroDelPunto(grosor)` = `PX_POR_GROSOR_EN_MANDO` (16) × grosor. El rango de grosor pasa de 0.4..2.2 a **0.5..3.5** (7:1, para que el punto vaya justo de 8 a 56 px): en la pared, el trazo fino va de 1.5 a 10.5 px y el aire de 4.5 a 31.5 px — un pincel gordo de verdad en 1280 px, no un matiz. Base sin cambio (1 → punto de 16 px). El botón sigue en 128 px fijos (el punto lo centra su rejilla interna) y la rejilla del mando no se toca; sin transición en el punto: sigue al dedo al instante y al soltar no cambia. `escalaDelPunto` desaparece; 3 pruebas nuevas de `diametroDelPunto` (16/8/56, medio camino = 36, fuera de rango se acota) y la prueba de "desde 1.6 bajar el arrastre completo" ya no fija 1.0 sino `1.6 − (GROSOR_BASE − GROSOR_MIN)`.

**Medido con Chrome real, los mismos cinco estados:** punto **16 px** en reposo; arrastrando arriba al tope **52.6 px** (56 × el `scale(0.94)` del botón presionado); **tras soltar 56 px** — se ve a simple vista que quedó grueso; arrastrando abajo desde ahí 45.1 px (48 × 0.94); tras soltar **48 px** (3.5 → 3.0, la regla relativa). Centro del botón 610.9 en reposo y tras soltar (igual); tarjetas, texto y presentes sin cambiar de posición ni de alto en ningún estado; 390/390. Cempasúchil: un renglón de 19.6 px, gap 7 px. Capturas rehechas con los mismos nombres (`mando-grosor-390x844-{reposo,arriba,abajo,tras-soltar,cempasuchil}.png`).

`npm run typecheck && npm run lint` (0 errores) `&& npm test` (795/795) `&& npm run build`: verde. Rama `pincel-grosor`, commit local, sin push.
