...
Con Chromium de `/opt/pw-browsers/chromium-1194` (`playwright-core` instalado con `npm i --no-save` en el scratchpad de la sesión, nunca en el repo). Aviso del gestor durante esta pieza: `document.fonts.ready` puede resolver antes de que Bricolage Grotesque llegue de verdad (le pasó al operador de OL-169) — así que **no** se usó eso solo: antes de cada una de las seis capturas, `page.waitForFunction` esperó (hasta 20 s) a que `document.fonts.check('700 20px "Bricolage Grotesque"')` **y** `document.fonts.check('400 16px "Bricolage Grotesque"')` fueran `true` a la vez (los dos pesos, no solo el de los títulos); las seis lo cumplieron dentro del plazo, sin necesitar la letra de reserva. Captura del elemento `.telefono` (390×844 exactos, sin `fullPage`, evitando el artefacto ya descrito en la bitácora 200); comprobado sin scroll horizontal (`scrollWidth === clientWidth === 390`) en las seis pantallas y estados; las seis abiertas con `Read` y miradas de cerca (números del reloj, "SMSNSTRS", los títulos) para confirmar que la letra es Bricolage Grotesque condensada y no Arial de reserva.
