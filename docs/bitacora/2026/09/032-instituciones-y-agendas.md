...
- **Tipos Museo y Escuela, en código y sin desplegar:** `TIPOS` suma los dos (chips en el orden Casa de cultura · Museo · Foro · Galería · Escuela · Colectivo · Biblioteca · Otro); `deducirTipo` los reconoce antes que centro cultural y teatro ("Escuela Estatal de Teatro" es escuela); el importador los usa. La migración `20260914120000_tipos_museo_escuela.sql` solo amplía la lista de la base, sin tocar filas. La reclasificación (13 museos y 7 escuelas, el IPBA incluido) vive en `scripts/instituciones/reclasificar-tipos.sql`, fuera de `supabase/migrations`, para que ningún `db:push` la aplique antes de tiempo. Orden obligado: migración → PR y despliegue → reclasificación (antes del despliegue, la app publicada no conoce los tipos nuevos).
...
## Pendiente
- Tipos Museo y Escuela: aplicar la migración (`npm run db:push`), PR y despliegue, y después la reclasificación.
- Subir esta pieza (scripts, documentos y tipos) en un PR cuando el founder lo pida.
- La Agenda Cultural de octubre de la Secretaría sale entre el 30 de septiembre y el 5 de octubre.
