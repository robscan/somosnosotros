...
export function extraerRegistros(html: string): Registro[] {
  ...
  let ultimoParrafo: string | null = null;
  const abrir = (nombre: string): Registro => {
    const r = nuevo(nombre, imagenPendiente);
    // En páginas a columnas el texto de una ficha puede venir antes de su encabezado: si la ficha abierta
    // tiene un párrafo igual al nombre nuevo (o que empieza con él), de ahí en adelante es de la nueva.
    if (actual) {
  ...
  for (const t of cuerpo.matchAll(TOKEN)) {
    if (t[1] !== undefined) {
      ...
      ultimoParrafo = null;
      if (!texto) continue;
      // El primer encabezado de nivel 1 es el título de la página; en algunas páginas los nombres también van en h1.
      if (Number(t[1]) === 1 && !tituloVisto) {
        tituloVisto = true;
        continue;
      }
      if (ES_ORIGEN.test(texto)) {
    ...
    if (t[3] !== undefined) {
      ...
      if (!texto) continue;
      // Nombre en negritas sin encabezado: un párrafo corto seguido del origen "(San Luis Potosí)".
...
/** Corto, sin punto final y sin verbo aparente: "SERES Arte Inclusivo", no "Surge en 1995 con la finalidad…". */
function pareceNombre(t: string): boolean {
  return t.length <= 80 && !/[.!?:;]$/.test(t) && t.split(" ").length <= 12;
}
...
function limpiarNombre(n: string): string {
  ...
  return t.slice(0, LIMITES_ARTISTA.nombre);
}

/** Corta en el final de una frase antes del límite; si no hay frase razonable, en una palabra con "…". */
export function recortarDescripcion(texto: string, maximo: number = LIMITES_ARTISTA.descripcion): string | null {
  const t = texto.trim();
  if (!t) return null;
...
function redesDe(enlaces: string[]): Enlace[] {
  return normalizarRedes(enlaces.filter((u) => !DE_GOOGLE.test(u)).map((url) => ({ url })));
}
...
export function separarDireccion(parrafos: string[]): { descripcion: string[]; direccion: string[] } {
  ...
  if (i >= 0) return { descripcion: parrafos.slice(0, i), direccion: parrafos.slice(i + 1).filter((p) => !ES_ORIGEN.test(p) && !NO_ES_NOMBRE.test(p)) };
...
/** Erratas del catálogo que impiden ubicar la calle (se revisaron a mano contra el mapa). */
const CORRECCIONES_DIRECCION: [RegExp, string][] = [
  [/independeiocnia/gi, "Independencia"],
  [/\brayon\b/gi, "Rayón"],
];
...
export function aLugar(r: Registro, pagina: Pagina): LugarCapo {
  ...
  const porNombre = deducirTipoLugar(nombre);
  let calle = direccion.join(", ").replace(/\s+,/g, ",").replace(/,{2,}/g, ",").replace(/,$/, "").trim();
  for (const [errata, bien] of CORRECCIONES_DIRECCION) calle = calle.replace(errata, bien);
  const conCiudad = calle && !/san luis potos|s\.?l\.?p\b/i.test(calle) ? `${calle}, San Luis Potosí, S.L.P.` : calle;
  return {
    nombre,
    tipo: porNombre !== "otro" ? porNombre : deducirTipoLugar(descripcion.join(" ")),
