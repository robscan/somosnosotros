...
export async function run({ as, check, expectError, query }) {
  ...
  const guardar = async (id, d = datos, p = null, quien = []) => (await query(
    "select public.guardar_evento_completo($1, $2::jsonb, $3::jsonb, $4::jsonb) as r", [id, JSON.stringify(d), p === null ? null : JSON.stringify(p), JSON.stringify(quien)],
  )).rows[0].r;
