...
-- bitácora avisó y dejó sin tocar otros sitios con el mismo patrón. Dos de ellos son el origen de L1 y L27 del founder
-- (2026-09-21): panel_eventos()/panel_fichas_conteos() (filtro 'comunidad': la lista y el badge detrás de la tarjeta
-- «Publica la comunidad») y tira_destacados() (el carril de Destacados, CTE `voy`) siguen comparando contra el rol de
-- HOY (perfiles.rol). El número (con rol_en) y la lista o el carril (con el rol de hoy) pueden salir de criterios
...
-- Solo cambia la comparación de rol en el cuerpo de las tres funciones; misma firma, mismo `security`/`search_path`
-- y mismos grants que hoy (tras el endurecimiento del Security Advisor, migración 20260918130000): panel_eventos() y
-- panel_fichas_conteos() son `security invoker` desde entonces, así que además necesitan poder llamar a rol_en(), hoy
-- revocada de `authenticated` — se le concede aquí, sin ampliar nada más (rol_en solo devuelve un rol, no identifica
-- a nadie, y panel_eventos()/panel_fichas_conteos() ya exigen public.es_admin() por dentro para devolver algo).
-- El tercer sitio que la bitácora 101 dejó documentado y sin tocar —panel_personas(), filtro 'nuevas'— es de la
-- pantalla Personas, no de la agenda ni de destacados: fuera del alcance de esta pieza (A2), queda documentado.
begin;

grant execute on function public.rol_en(uuid, timestamptz) to authenticated;
...
language sql stable security invoker set search_path = '' as $$
  ...
  where public.es_admin()
    ...
    and case coalesce(p_filtro, 'proximos')
      ...
      when 'comunidad' then e.visible and (p.id is null or public.rol_en(p.id, e.creado_en) <> 'admin')
  ...
  select case
    ...
    when p_tipo = 'eventos' then json_build_object(
      ...
      'comunidad', (select count(*) from public.eventos e left join public.perfiles p on p.id = e.creado_por
        where e.visible and e.termina >= now() and (p.id is null or public.rol_en(p.id, e.creado_en) <> 'admin')),
