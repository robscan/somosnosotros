 ...
 * "no-soportado" lleva el detalle técnico (OL-131, punto 3 del encargo): sin la llave pública (nuestro) y sin
 * las APIs del navegador (suyo) piden arreglos distintos, y antes se veían igual.
 */
function subtitulo(estado: EstadoPush | null, p: Plataforma | null, llavePush: string): string {
  const aqui = enEste(p);
  switch (estado) {
    case "encendido":
    ...
    case "no-soportado": {
      const detalle = detalleNoSoportado(llavePush);
      return detalle ? `Este navegador no los recibe (${detalle})` : "Este navegador no los recibe";
    }
    default:
      return " ";
  }
