...
export default async function Admin() {
  ...
  const [{ data: reportes }, { data: lugares }, { data: eventos }, perfiles, asistencias, seguimientos] = await Promise.all([
    supabase.from("reportes").select("id, tipo, objeto_id, motivo, detalle, creado_en, autor:perfiles!reportes_creado_por_fkey(nombre)").eq("atendido", false).order("creado_en", { ascending: false }).limit(50),
  ...
  const rutaDe = (r: Reporte) => (r.tipo === "lugar" ? `/lugares/${r.objeto_id}` : r.tipo === "evento" ? `/eventos/${r.objeto_id}` : `/personas/${r.objeto_id}`);
