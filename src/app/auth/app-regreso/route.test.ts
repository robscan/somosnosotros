...
/**
 * Ruta puente para la vuelta del envoltorio de iPhone (OL-194, corrección de seguridad): confirma el enlace mágico
 * de un solo uso que arma /auth/[proveedor]/fin (urlAppTrasEntrar) y solo entonces deja pasar a `siguiente`; ver el
 * comentario en route.ts. Estas mismas pruebas cubren el caso de que alguien abra esta URL fuera de la app (un
 * navegador cualquiera): la ruta no distingue de dónde viene la petición, así que la sesión siempre queda en quien
 * la abrió, nunca en otra app.
 */
it("con un token_hash válido, confirma el enlace (deja la sesión en cookies) y sigue a siguiente", async () => {
  ...
  const respuesta = await GET(new NextRequest("https://somosnosotros.org/auth/app-regreso?token_hash=el-token&siguiente=%2Fperfil"));
...
it("con un token_hash inválido o vencido, vuelve a Entrar con un aviso, no deja a la persona varada", async () => {
  ...
  cliente.mockResolvedValue({ auth: { verifyOtp } } as unknown as NonNullable<Awaited<ReturnType<typeof clienteServidor>>>);
  const respuesta = await GET(new NextRequest("https://somosnosotros.org/auth/app-regreso?token_hash=vencido&siguiente=%2Fperfil"));
...
it("sin token_hash no llama a Supabase: vuelve a Entrar con un aviso directo", async () => {
  expect((await GET(new NextRequest("https://somosnosotros.org/auth/app-regreso"))).headers.get("location")).toBe("https://somosnosotros.org/entrar?error=enlace");
