...
export default async function CarrilTusPlanes({ personaPromise, avisos, verTodosHref }: { personaPromise: Promise<Persona | null>; avisos: AvisosLista | null; verTodosHref: string }) {
  ...
  return <CarrilEventosCliente tarjetas={eventos.map((e) => tarjetaEvento(e, ahora))} asistencias={asistencias} avisos={avisos} titulo="Tus planes" tamano="mediana" memoria="inicio-tus-planes" verTodosHref={verTodosHref} />;
}
