...
export default async function CarrilEntidad({ promise, seguidosPromise, que, avisos, titulo, memoria, verTodosHref, grande = false }: { promise: Promise<Tarjeta[]>; seguidosPromise: Promise<string[] | null>; que: "lugar" | "artista"; avisos: AvisosLista | null; titulo: string; memoria: string; verTodosHref: string; grande?: boolean }) {
  ...
  return <CarrilEntidadCliente tarjetas={tarjetas} que={que} seguidos={seguidos} avisos={avisos} titulo={titulo} memoria={memoria} verTodosHref={verTodosHref} grande={grande} />;
}
