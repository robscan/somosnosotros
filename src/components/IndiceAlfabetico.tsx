...
export default function IndiceAlfabetico({ letra, href, onSeleccionar, hrefTodos, onQuitar }: { letra: string | null; href?: (letra: string) => string; onSeleccionar?: (letra: string) => void; hrefTodos?: string; onQuitar?: () => void }) {
  return (
    <nav className={styles.indice} aria-label="Ir a una letra">
      {hrefTodos ? <Link href={hrefTodos} scroll={false} replace aria-label="Ver todas las letras" className={!letra ? styles.activa : undefined}>Todos</Link> : <button type="button" onClick={onQuitar} aria-label="Ver todas las letras" className={!letra ? styles.activa : undefined}>Todos</button>}
